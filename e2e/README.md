...
- [`accessibility.spec.ts`](scenarios/accessibility.md) — axe WCAG A/AA smoke: login + 25 authed pages (incl. the kudos/feedback/days-off create forms, `/feature-flags`, the v2.36.0 impact-log pair, and the v2.42.0 succession pair); `color-contrast` consciously waived.
...
- [`succession.spec.ts`](scenarios/succession.md) — succession plans (v2.42.0): create for a report, nominate a successor with the modal-created linked development goal, the under-bench cue, the one-primary confirm-demote on a second PRIMARY nomination (v2.43.0), invisibility to the seat's person, close (read-only) + delete.
