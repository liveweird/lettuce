...
- [`days-off.spec.ts`](scenarios/days-off.md) — entries with no approval lifecycle (v3.9.0 — every entry is active the moment it's created), the manager-set yearly allowance on the drill-down (v2.32.0 — the admin edit page lost the field), the owner-or-chain-manager Delete right, the create/delete notification fan-out to the owner's whole team + their direct manager (a teammate's bell, not just the manager's), calendar, budgets + corrections, the manager's kept on-behalf recording (v2.29.0, no longer "auto-accepted"), and the paid pools (v3.2.0): the admin's pool-kind registry, the manager's Add pool grant + archive on the drill-down, and an entry booked from the extra pool via the Type picker.
