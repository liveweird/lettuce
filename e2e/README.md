...
- [`days-off.spec.ts`](scenarios/days-off.md) — entries with no approval lifecycle (v3.9.0 — every entry is active the moment it's created), the manager-set yearly allowance on the drill-down (v2.32.0 — the admin edit page lost the field), the owner-or-chain-manager Delete right, the create/delete notification fan-out to the owner's whole team + their direct manager (a teammate's bell, not just the manager's), calendar, budgets + corrections, the manager's kept on-behalf recording (v2.29.0, no longer "auto-accepted"), the paid pools (v3.2.0): the admin's pool-kind registry, the manager's Add pool grant + archive on the drill-down, and an entry booked from the extra pool via the Type picker, and the calendar/Team-tab `includeIndirect` widening to an indirect chain manager, with the report's team badge (v3.13.0).
- [`days-off-tutorial.spec.ts`](scenarios/days-off-tutorial.md) — the "How days off work" guided tutorial (v3.16.0): the manager's 12-step and the non-manager's 8-step read-only walk, landmark order, and the return to the Days off page with the caller's own days-off total unchanged.
- [`dictionaries.spec.ts`](scenarios/dictionaries.md) — the whole-list dictionary editor (add/reorder/rename, multilingual — EN required, translations optional), read-only EN fallback, and compact aligned rows across all four dictionaries in EN/PL at desktop/mobile widths, including long values and translation popovers.
...
- [`tutorials-menu.spec.ts`](scenarios/tutorials-menu.md) — the account menu's Tutorials submenu (v4.4.0): the role-filtered list (no succession for a non-manager), a launch from another page opening the tutorial's hub first, a full non-manager goals walk back to Goals and a manager's succession launch from Kudos, with no API write throughout.
- [`hr.spec.ts`](scenarios/hr.md) — the HR auditor reads a private draft via the Audit section + the guarded career timeline (2026-08) + all four remaining drill-downs (reviews, days off incl. the read-only paid-leave budget, impact log, succession — v3.24.0) + the team-KPI auditor entry points (the All-teams tab and the team-details link, v3.24.0) + the org-wide calendar scope (v3.25.0); admin gets no audit surface; plus a read-only smoke on the seeded `hr@lettuce.local` demo account (v4.1.0) reaching the Audit section with no admin surface; plus a read-only walk of the org-wide `/performance?tab=managed` "Everyone (auditor)" reports scope and all three pulse tabs (Results/Trend/Participation) for a relationship-less HR account (v4.3.0).
...
- [`list-layout.spec.ts`](scenarios/list-layout.md) — responsive list regression: long/multi-party feedback in EN/PL at 1440/1280/1024/390, long Users/Teams values and paired-feedback tabs, narrow sorting and actions, retained local calendar scrolling, plus the Team's-performance matrix fitting a 1280px laptop with no scroll in EN/PL (v3.11.1 rotated rating headers) while its rating pills still render their digits unclipped (v3.25.1) and no status pill splits a word (v4.3.1), and a dashboard subordinate card's "Last review" dot+period staying on one line at 1440px (v4.x).
...
- [`notification-preferences.spec.ts`](scenarios/notification-preferences.md) — the v4.0.0 per-type in-app/email preference matrix: muting one type mints no bell row while an unmuted type still arrives, the master "Send me emails" switch round-trips and greys out the email column while off, and the admin-for-another-user branch.
- [`teams-notifications.spec.ts`](scenarios/teams-notifications.md) — the v4.5.0 Microsoft Teams channel against the compose `teams-stub` (WireMock): the opt-in per-user flag reveals the Teams preferences column, a locked-type notification arrives as a direct message in the stub journal, and an uninstalled-app user (403) gets none while the bell row still lands (stub-gated).
- [`notifications.spec.ts`](scenarios/notifications.md) — bell mechanics: badge, seen/unseen, mark all, delete.
- [`one-on-ones.spec.ts`](scenarios/one-on-ones.md) — documenting 1:1s, action-item carry-over, subordinate notification, the measured pin that the read-only document's action items render as a table, not stacked cards, at desktop width (v3.25.2), the managed list's "Latest 1:1 only" filter narrowing a pair's two live meetings down to the newest, and the edit screen's "New 1:1" button saving in-progress notes (Save and continue) before landing on the create screen with the same person locked in.
...
- [`performance-reviews-tutorial.spec.ts`](scenarios/performance-reviews-tutorial.md) — the "How performance reviews work" guided tutorial (v3.17.0): the manager's 14-step and the non-manager's 6-step read-only walk, landmark order, and the return to the Performance page with no non-GET API request issued during the walk (a fresh database's empty period timeline is filled once by ADMIN as a precondition).
...
- [`tour.spec.ts`](scenarios/tour.md) — the guided tour's landmark order as manager (22 steps) and admin (21 steps), menu-only, no navigation (v3.23.0).
