...
test("a manager walks a goal around the whole lifecycle: draft, activate, progress, archive, reopen", async ({ page }) => {
  ...
  await rowByTitle(page, title).getByRole("link", { name: `Edit goal ${title}` }).click();
  await expect(page).toHaveURL(new RegExp(`/goals/${id}/edit`));
  ...
  await rowByTitle(page, title).getByRole("link", { name: `Update goal ${title}` }).click();
  await expect(page).toHaveURL(new RegExp(`/goals/${id}/edit`));
  await page.getByLabel("Current").fill("3");
  await page.getByLabel("Comment (optional)").fill("Three of five shipped");
  await page.getByRole("button", { name: "Save", exact: true }).click();
  await expect(page).toHaveURL(/\/users\/\d+\/goals/);
  await expect(rowByTitle(page, title).getByText("3", { exact: true })).toBeVisible();

  // Archive from the row's Lifecycle ▾ menu — the summary is mandatory.
  await rowByTitle(page, title).getByRole("button", { name: `Lifecycle actions for goal ${title}` }).click();
  ...
  await expect(rowByTitle(page, title).getByText("Archived", { exact: true })).toBeVisible();
...
test("a PLAN goal: milestones defined in draft, ticked on the update screen, struck through when done", async ({ page }) => {
  ...
  await expect(rowByTitle(page, title).getByText("0 / 2", { exact: true })).toBeVisible();
  ...
  await expect(page).toHaveURL(/\/users\/\d+\/goals/);
  await expect(rowByTitle(page, title).getByText("1 / 2", { exact: true })).toBeVisible();
...
test("the Goals-I've-set tab: Reports widens from own goals to goals set down the chain", async ({ page }) => {
  ...
  await expect(rowByTitle(page, title).getByRole("link", { name: `View goal ${title}` })).toBeVisible();
  await expect(rowByTitle(page, title).getByRole("link", { name: `Edit goal ${title}` })).toHaveCount(0);
  await expect(rowByTitle(page, title).getByRole("link", { name: `Update goal ${title}` })).toHaveCount(0);
  await expect(
    rowByTitle(page, title).getByRole("button", { name: `Lifecycle actions for goal ${title}` }),
  ).toHaveCount(0);
