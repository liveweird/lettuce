...
- **Since**: v2.15.0 (manager-managed position timeline; the admin form lost its career fields),
  ...
  are self/chain/HR-only — the seniority-privacy round), v2.39.0 (past inserts backfill history;
  the sameness note follows the drafted date's neighbors and a taken date is flagged)
...
6. They start a second position dated 2025-02-01 — the form prefills from the current position
   ...
   repeats a neighboring position exactly (v2.15.2: the triple must differ). Changing the
   seniority to a different value enables the submit, and they start the position.
...
9. They backfill a position in the past (v2.39.0). First they try the survivor's own start date
   (2024-03-01) — the form flags that another position already starts on that date and keeps the
   submit disabled. They change the date to 2023-06-01 — now the prefilled triple repeats its
   date-neighbor (the survivor), so the sameness note shows; changing the seniority enables the
   submit, and they record the position.
   - *Expected*: the backfilled position (started 2023-06-01) sits on the timeline closed — the
     survivor stays the one **Current** position.
10. They open the subordinate's user-details view.
    - *Expected*: the current position's career-path value backs the Profile section.
11. Still as the manager, they open the nav **Career** page (v2.16.0).
    - *Expected*: the **My career** tab shows their own (empty) timeline — "No positions recorded
      yet."
12. They switch to the **Team pyramid** tab.
    - *Expected*: exactly one row for the subordinate, carrying the surviving position's value.
13. Via the filters they widen **Reports** to "All reports (including indirect)".
    - *Expected*: the pyramid re-fetches with the wider scope; the subordinate's row is still the
      one row.
14. They nudge the time slider one day back (v2.17.0), then click **Today**.
    - *Expected*: the "As of …" badge appears for the past date and disappears on Today. (The
      ...
      today.)
15. They flip the pyramid to the **Chart** view.
    - *Expected*: the distribution chart replaces the table.
16. The manager signs out; the subordinate signs in and opens the notification bell.
    - *Expected*: a notification that a new position was recorded in their career progression.
17. The subordinate opens their own career timeline.
...
20. The admin **removes** the throwaway entry from the dictionary and saves (the cleanup half of
    the append idiom), then signs out; the subordinate signs back in and opens their own timeline.
    - *Expected*: the retired entry keeps resolving on the subordinate's timeline — BOTH rows
      (the survivor and the step-9 backfill) show the renamed value, because positions
