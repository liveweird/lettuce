...
## Scenario: the guided tour walks all 53 manager steps in the documented order
...
2. They start the tour via "Replay this tour" and click "Next" through every step until "Done".
   - *Expected*: exactly 53 steps. Audience math over the 56 total steps: Manager AAA is a
...
2. Before walking, the expected step count is derived from whether the admin currently manages
   ...
   manager-gated steps (the eight manager-only tab steps plus the manager-or-HR Pulse
   participation step) on top of the baseline 47.
3. They start the tour via "Replay this tour" and click "Next" through every step until "Done".
   - *Expected*: 47 steps for the pristine seed admin (an ADMIN who manages no team — the
     manager-only tab steps drop out), or 56 when the dev database gives them a managed team.
