...
- **Language probe:** switch via the header Language menu (or `PUT /api/v1/users/{id}/language`). Since v2.21.0 the language is a server-stored per-user property that `persistSession` re-applies on login/refresh — a hand-set `localStorage.lettuce.lang` + reload works only until the next silent refresh reverts it to the account's stored language, so don't use it for signed-in probes.
