...
- **SPA**: nav leaf **"Succession plans"** (`IconUserShield`, `feature: "SUCCESSION_PLANS"`,
  ...
  **Nominations** (the bench badge, the Paper cards with linked-goal chips → `goalViewLink`;
  ...
  `succession.own`/`succession.team`/`userSuccession.audit`. **Feature tutorial (v3.22.0).** `tutorials/succession.tsx` — the read-only "How succession plans work" walkthrough (`home` `/succession?tab=own`; 5 steps for a non-manager who opens the hub by URL — intro, My plans, its Filters, the access model, the closing bench-cue step — 13 for a manager: + My subordinates' plans, New plan, the create form's Seat & criticality and Loss impact sections and its footer, nominations and the Review screen as body steps, the My subordinates card): anchors `succession-own`/`succession-team` (the tabs), `succession-filters` (`SuccessionPlanTable.tourId` → the FilterPanel), `succession-new` (the header button), `succession-form-seat`/`succession-form-loss-impact` (`SuccessionPlanFields`' tour-id props → wrapper Boxes around the two Fieldsets; the Review screen passes none) and `succession-form-actions` (the create footer) plus the dashboard's `dashboard-subordinates`; the create URL never carries a person (a picked person arms the discard guard); launcher `TutorialButton` (`succession-tutorial`) in the hub's PageHeader; tests `tutorials/succession.test.tsx` + the launcher case in `SuccessionPlans.test.tsx`; e2e `succession-tutorial.spec.ts` (owns nothing, the `recordApiWrites` oracle).
