...
All under `/api/v1/pulse-surveys/*` (tag `pulse-surveys`, 17 operations); every handler resolves `pulseCaller()` FIRST (uniform 403 for a disabled caller). The cycle list is an **unpaged registry** (`{items}`, newest first — a cadence of weeks caps volume; deliberate deviation from the paged-list convention, like review-periods). `rotatingQuestionEn`/`rotatingQuestionPl` on cycle reads is null for non-admins until OPEN/CLOSED (unopened questions stay unspoiled; CANCELLED reveals nothing to non-admins); the list's `participantCount`/`responseCount` are ADMIN-only enrichment. `DashboardSummary` gained `pulseOpenCloseDate`/`pulseSubmitted` (null unless the caller is a participant of an OPEN cycle) backing the SPA tile.
