...
import { partyDisplayName } from "./userDisplay";
...
const t = ((key: string, opts?: Record<string, unknown>): string => {
  ...
  if (key === "common.deletedSuffix") return `${opts?.name} (deleted)`;
  return key;
}) as unknown as TFunction;

describe("partyDisplayName", () => {
  test("returns 'You' when the user is the current user", () => {
    expect(partyDisplayName(7, "Alice", false, 7, t)).toBe("You");
  });

  test("'You' wins even when the (impossible) self row is flagged deleted", () => {
    expect(partyDisplayName(7, "Alice", true, 7, t)).toBe("You");
  });

  test("returns the plain name for another, non-deleted user", () => {
    expect(partyDisplayName(10, "Alice", false, 7, t)).toBe("Alice");
  });

  test("suffixes the name for a deleted other user", () => {
    expect(partyDisplayName(10, "Alice", true, 7, t)).toBe("Alice (deleted)");
  });

  test("returns an em dash when there is no name", () => {
    expect(partyDisplayName(null, null, false, 7, t)).toBe("—");
  });

  test("does not substitute 'You' when there is no current user id", () => {
    expect(partyDisplayName(7, "Alice", false, null, t)).toBe("Alice");
  });

  test("does not substitute 'You' when the user id is missing", () => {
    expect(partyDisplayName(null, "Alice", false, 7, t)).toBe("Alice");
  });
});
