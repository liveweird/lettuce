...
// Design language (v1.35.0, "clean enterprise SaaS"): the brand green is the interactive
// accent (buttons, links, active nav, subtle row actions) at a deep, serious shade; SEMANTIC
// success is teal (see the status badges) so state colors never impersonate the brand; the
// canvas is a quiet near-white (dark: dark-8) that lets white surfaces lift on a soft, diffuse
// shadow scale. Don't reintroduce stock-blue actions or stock-green success states.
export const theme = createTheme({
  ...
  headings: {
    fontFamily: sans,
    fontWeight: "600",
    // Tighter than Mantine's defaults — 40 pages title themselves with order={2}, and the
  ...
  // Soft, diffuse elevation (the modern-SaaS ambient look) — every existing `shadow="sm"`
  // Paper picks this up with zero call-site changes.
  shadows: {
