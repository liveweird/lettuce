...
describe("DashboardHero", () => {
  ...
  test("the received tile carries the trend as a hint icon with the full comment", async () => {
    setupMocks({ ...MANAGER_SUMMARY, feedbackReceivedPrev30d: 2 });
    renderHero();

    // 5 now vs 2 before → "+3 vs previous 30 days", tooltip text doubling as the aria-label.
    expect(await screen.findByLabelText("+3 vs previous 30 days")).toBeInTheDocument();
    // The bare value still renders separately (locator stability), with no visible sub-line.
    expect(screen.getByText("5")).toBeInTheDocument();
    expect(screen.queryByText("vs previous 30 days")).toBeNull();
  });

  test("a negative delta renders signed; a missing field renders no hint (older server)", async () => {
    setupMocks({ ...MANAGER_SUMMARY, feedbackReceivedPrev30d: 9 });
    renderHero();
    expect(await screen.findByLabelText("-4 vs previous 30 days")).toBeInTheDocument();

    // Without the field (MANAGER_SUMMARY carries none) the tile shows no hint icon at all.
    // within(container): the render-result queries bind to document.body, where the first
    // hero (with its hint) is still mounted.
    setupMocks(MANAGER_SUMMARY);
    ...
    expect(
      within(second.container as HTMLElement).queryByLabelText(/vs previous 30 days/),
    ).toBeNull();
  });
