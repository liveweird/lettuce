...
export type BulkFeatureUpdate<T> = {
  ...
  run: () => Promise<void>;
  /** Re-run only the failed rows (v2.24.0 — a partial failure is retryable, not a dead end). */
  retry: () => Promise<void>;
  cancel: () => void;
};
 ...
 * fail KEEP THEIR IDENTITY in `failed` (v2.24.0) so the page can name them and offer `retry`.
