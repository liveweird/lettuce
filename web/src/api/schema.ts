...
export interface paths {
    ...
    "/api/v1/login": {
         ...
         *     fails closed with `503`. An account with too many live pending challenges already
         *     outstanding (default 5, `MFA_MAX_PENDING_CHALLENGES`) instead answers `429` — no new
         *     code is emailed.
         */
    ...
    "/api/v1/days-off/calendar": {
         ...
         *     - `scope=org` (v3.25.0): **HR only** (`403` for anyone else, ADMIN included — the
         *       `view=all` rule of the team-KPI list), audit-logged as `hr.list`. Optionally narrowed
         *       to one team with `teamId`. Unlike the caller-relative scopes it lists **only the
         *       people with at least one active entry in the month**, never the whole roster: an
         *       auditor scans absences rather than a roster, and it keeps an unpaged payload bounded
         ...
         *     the caller on `member`, and the person's own teams on `org`), so a widened calendar
         *     still says where each person sits.
         *
