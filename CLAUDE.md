...
├── infra/crypto/       field-level encryption: FieldCipher (AES-256-GCM) + configureCrypto + the EncryptedAtRest interface the nine encrypted services implement (see "Encryption at rest" in `.claude/docs/security.md`)
...
- **Feedbacks** (`feedbacks/`, SPA feedback pages) → `.claude/docs/features/feedbacks.md` — the five-status lifecycle (REQUESTED/DRAFT/SENT/WITHDRAWN/REJECTED) + POST transition actions, create invariants (no-duplicate 409, provider ≠ subject since v2.36.0 — legacy self-reflection rows stay functional — requester message), the four list views, and the `feedback_events` audit trail.
...
- **Migration catalog** → `.claude/docs/features/migrations.md` — the per-migration V1–V66 history. Read it before adding a migration or reasoning about schema history.
