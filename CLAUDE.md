...
- **Succession plans** (`succession/`, SPA `Succession*`) → `.claude/docs/features/succession-plans.md` — the manager's private critical-role/seat records (v2.42.0): criticality/retention-risk labels, encrypted ordered loss-impact/competency-gap lists, the bench-depth cue (ALL nominations count), successor nominations with linked development goals, owner-only writes / chain-above-the-owner+HR reads, INVISIBLE to the seat's person and candidates, no notifications, OPEN→CLOSED terminal.
