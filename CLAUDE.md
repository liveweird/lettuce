...
- **Scope:** frontend chrome is translated, and **feedback history events** are localized client-side (the server stores them structurally, with no rendered string — see "Feedback events"). Server-generated **notification** message bodies (`FeedbackNotifications.kt`) still stay English (a possible later follow-up using the same structured-event pattern).
