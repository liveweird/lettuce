...
- **Feedbacks** (`feedbacks/`, SPA feedback pages) → `.claude/docs/features/feedbacks.md` — the five-status lifecycle (REQUESTED/DRAFT/SENT/WITHDRAWN/REJECTED) + POST transition actions, create invariants (no-duplicate 409, self-reflection, requester message), the three list views, and the `feedback_events` audit trail.
...
- **Performance reviews** (`reviews/`, SPA `PerformanceReview*`/`Performance*`) → `.claude/docs/features/performance-reviews.md` — DRAFT ↔ CALIBRATION ↔ PUBLISHED, four rated categories (1–6 + encrypted summaries), the append-only/gapless review-period registry, one review per (subordinate, period), completeness gates.
...
- **Migration catalog** → `.claude/docs/features/migrations.md` — the per-migration V1–V43 history. Read it before adding a migration or reasoning about schema history.
