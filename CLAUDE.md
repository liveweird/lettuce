...
Current migrations are `V1`–`V14`: schema for users/teams/feedbacks/revoked-tokens, the `users.role` column (`V5`), the `admin@lettuce.local` seed (`V6`), a soft-delete `marked_as_deleted BOOLEAN` column (with index) on `users` (`V7`) and `teams` (`V8`), a demo-org seed (`V9`), a feedback templates table (`V10`), the `REJECTED` feedback status (`V11`), a feedback `last_modified` column (`V12`), the `notifications` table (`V13`), a seed of four default feedback templates (`V14`, idempotent via `ON CONFLICT (name)`), the `feedback_events` audit table (`V15`, `ON DELETE CASCADE` from `feedbacks`), a soft-delete `marked_as_deleted` column (with index) on `templates` (`V16`) and `notifications` (`V17`). Soft delete is the pattern for **users, teams, templates, and notifications** — rows are flagged `marked_as_deleted`, not physically removed; every read/list/count and the `delete`/`update` mutations filter on `marked_as_deleted = false` (the per-service `active()` helper), and `delete` flips the flag and returns the affected-row count (`0` → route 404). Unique fields differ: users (`email`) and teams keep the unique value **reserved** after soft-delete (global unique constraint), whereas `templates` swap their global `UNIQUE(name)` for a **partial unique index** `uq_templates_name_active` (`V16`, `WHERE marked_as_deleted = false`) so a deleted template's name can be reused. Only `feedbacks` (hard delete, route pre-read drives 404) remains physically deleted.
