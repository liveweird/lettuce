...
- Dependency-family alignment guard: `./gradlew :server:checkDependencyAlignment` — rides `check`; fails when a family that must move as one (`io.netty`, `io.opentelemetry` incl. its `-alpha` artifacts, kotlin-stdlib/kotlin-reflect) resolves to several versions on the server runtime classpath, or when any `io.projectreactor*` module resolves to a version other than the one the Reactor BOM declares (its members legitimately carry different version numbers — reactor-core 3.8.7 / reactor-pool 1.2.7 / reactor-netty 1.3.7 — so the BOM is the pairing table, checkup #37 C4) — the `netty`, `opentelemetry` and `reactor-bom` notes in `gradle/libs.versions.toml` explain the pins.
...
├── auth/               POST /api/v1/login (+ /login/mfa — the v2.4.0 opt-in email second factor, MfaChallenges store), /api/v1/refresh, /api/v1/logout, /api/v1/password-reset + token minting + password hashing + LoginThrottle/PasswordResetThrottle (all three DB-backed since v3.11.0/V81 — `login_lockouts`/`password_reset_requests`/`mfa_challenges`, shared by replicas)
...
├── daysoff/            /api/v1/days-off/* entries (create + delete, no approval lifecycle since v3.9.0) + calendar (member/direct/chain scopes since v3.13.0) + budgets (one row per (user, paid pool) since v3.2.0) + the chain-manager allowance PUT (a per-pool upsert) + the pools DELETE (archive) + the ADMIN pool-types registry + corrections sub-resource + /api/v1/public-holidays registry + DaysOffService/PublicHolidayService (see "Days off" in `.claude/docs/features/days-off.md`)
...
├── notifications/      /api/v1/notifications/* list + read + seen/unseen + delete + NotificationService + Notifications table (recipient-scoped; list/total also exclude the recipient's disabled-feature types; every mint is also mirrored by email via NotificationEmailer; seen/user-deleted rows older than 30 days are hard-deleted by the on-mint purge — see "Email mirror" and "Retention" in `.claude/docs/features/notifications.md`) + the per-user, per-channel notification preferences (v4.0.0 — NotificationPreferenceService + user_notification_preferences, routed from users/UserRoutes.kt's `{id}/notification-preferences`; suppresses IN_APP at mint and EMAIL at send, PASSWORD_CHANGED locked on — see "Preferences" in `.claude/docs/features/notifications.md`)
...
- **Performance reviews** (`reviews/`, SPA `PerformanceReview*`/`Performance*`) → `.claude/docs/features/performance-reviews.md` — DRAFT ↔ CALIBRATION ↔ PUBLISHED, five rated categories (1–6 ratings + summaries — all ten encrypted at rest: summaries since V35, ratings since V45, aptitude since V79), the append-only/gapless review-period registry, one review per (subordinate, period), completeness gates.
- **Days off** (`daysoff/`, SPA `DaysOff*`) → `.claude/docs/features/days-off.md` — no approval lifecycle since v3.9.0 (create/delete only, no accept/reject/cancel), frozen half-day cost math, **the paid pools (v3.2.0/V74 — an ADMIN registry of pool kinds with a per-kind carry-over flag, per-user grants a chain manager adds/archives, every PAID entry and correction pinned to one pool)**, closed-form carry-over budget (per pool; non-carry kinds reset yearly) + corrections, public-holiday registry, calendar-parity reads.
...
- **Microsoft Teams notifications** (`infra/teams/`, `notifications/NotificationTeamsSender.kt`, the preferences page's Teams column) → `.claude/docs/features/teams-notifications.md` — the third notification channel (v4.5.0): a notification-only bot's proactive 1:1 messages, the deployment switch `teams.transport` + the opt-in per-user `TEAMS_NOTIFICATIONS` flag (V85, the MFA inverted default), Graph email → Entra-id resolution cached in `user_teams_identities`, the send-time skip order, production URL pinning, the local `teams-stub` (WireMock), and the real-tenant setup runbook.
...
- **Migration catalog** → `.claude/docs/features/migrations.md` — the per-migration V1–V85 history. Read it before adding a migration or reasoning about schema history.
