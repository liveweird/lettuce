...
├── daysoff/            /api/v1/days-off/* entries (create + delete, no approval lifecycle since v3.9.0) + calendar + budgets (one row per (user, paid pool) since v3.2.0) + the chain-manager allowance PUT (a per-pool upsert) + the pools DELETE (archive) + the ADMIN pool-types registry + corrections sub-resource + /api/v1/public-holidays registry + DaysOffService/PublicHolidayService (see "Days off" in `.claude/docs/features/days-off.md`)
