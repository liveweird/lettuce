...
class TeamMemberPeerStatsTest {
    ...
    fun `stats are null without data and the pair is never populated for the other views`() = testApplication {
        ...
        val mgrId = TestUsers.seed(email = mgrEmail, password = "pw", name = "Mgr", role = UserRole.USER)
        val subId = TestUsers.seed(email = uniqueEmail("sub"), password = "pw", name = "Sub", role = UserRole.USER)
    ...
    fun `lastFeedbackGivenAt is the SENT moment and survives later edits`() = testApplication {
        ...
        val callerId = TestUsers.seed(email = callerEmail, password = "pw", name = "Caller", role = UserRole.USER)
        val peerId = TestUsers.seed(email = uniqueEmail("peer"), password = "pw", name = "Peer", role = UserRole.USER)
        val mgrId = TestUsers.seed(email = uniqueEmail("mgr"), password = "pw", name = "Mgr", role = UserRole.USER)
    ...
    fun `lastFeedbackReceivedAt is the SENT moment and is visibility-scoped`() = testApplication {
        ...
        val peerId = TestUsers.seed(email = peerEmail, password = "pw", name = "Peer", role = UserRole.USER)
        val mgrId = TestUsers.seed(email = uniqueEmail("mgr"), password = "pw", name = "Mgr", role = UserRole.USER)
    ...
    fun `pre-audit-trail feedbacks fall back to lastModified and shared teams repeat the stats`() = testApplication {
        ...
        val callerId = TestUsers.seed(email = callerEmail, password = "pw", name = "Caller", role = UserRole.USER)
        val peerId = TestUsers.seed(email = uniqueEmail("peer"), password = "pw", name = "Peer", role = UserRole.USER)
        val otherPeerId = TestUsers.seed(email = uniqueEmail("peer2"), password = "pw", name = "Peer2", role = UserRole.USER)
        val mgrId = TestUsers.seed(email = uniqueEmail("mgr"), password = "pw", name = "Mgr", role = UserRole.USER)
        val admin = authedClient(adminEmail, "pw")
    ...
    fun `the stats are symmetric between two peers`() = testApplication {
        ...
        val aId = TestUsers.seed(email = aEmail, password = "pw", name = "PeerA", role = UserRole.USER)
        val bEmail = uniqueEmail("peer-b")
        val bId = TestUsers.seed(email = bEmail, password = "pw", name = "PeerB", role = UserRole.USER)
        val mgrId = TestUsers.seed(email = uniqueEmail("mgr"), password = "pw", name = "Mgr", role = UserRole.USER)
