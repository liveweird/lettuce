...
/**
 * The email-MFA login flow end to end (v2.4.0): challenge instead of tokens for an MFA-enabled
 ...
 * uniform 401s, deactivation between the steps, and the fail-closed 503 on a mail-less
 * deployment. The pure challenge-store unit lives in MfaChallengesTest; the inverted-default
 * flag semantics in FeatureFlagsTest/AuditTest.
 */
class MfaLoginTest {
    ...
    private suspend fun HttpClient.verify(challengeId: String, code: String): HttpResponse =
        post("/api/v1/login/mfa") {
            contentType(ContentType.Application.Json)
            setBody(MfaVerifyRequest(challengeId, code))
        }
    ...
    private suspend fun ApplicationTestBuilder.seedMfaUser(email: String, password: String): UInt {
        ...
        val userId = TestUsers.seed(email = email, password = password, roles = emptySet())
    ...
    fun `an MFA-enabled login answers a challenge, the emailed code mints a working pair, and the challenge is single-use`() =
        testApplication {
            ...
            try {
                ...
                // The pair is a real session: the bearer works.
                val me = client.get("/api/v1/notifications") {
    ...
    @Test
    fun `a user deactivated between the two steps gets the login 403, not tokens`() = testApplication {
        usePostgresTestcontainer()
        val adminEmail = uniqueEmail("mfa-admin")
        TestUsers.seed(email = adminEmail, password = "pw-123456789")
        val admin = authedClient(adminEmail, "pw-123456789")
        val email = uniqueEmail("mfa-deactivated")
        ...
        try {
            ...
            val challenge = client.login(email, "pw-123456789").body<MfaChallengeResponse>()
            val code = mail.codeFor(email)
            assertEquals(HttpStatusCode.NoContent, admin.post("/api/v1/users/$userId/deactivate").status)
            assertEquals(HttpStatusCode.Forbidden, client.verify(challenge.challengeId, code).status)
        } finally {
            mail.detach()
        }
    }
    ...
    @Test
    fun `an MFA-disabled account keeps the classic single-step login, byte-identical`() = testApplication {
        usePostgresTestcontainer()
