...
class AuthorizationTest {
    ...
    @Test
    fun `non-admin cannot assign career profile fields, even with an invalid payload`() = testApplication {
        usePostgresTestcontainer()
        val email = uniqueEmail("career-plain")
        val id = TestUsers.seed(email = email, password = "pw-123456789", roles = emptySet())
        val client = authedClient(email, "pw-123456789")
        val marker = java.util.UUID.randomUUID().toString().take(8)
        val (pathId) = TestDictionaries.append(
            ch.nokillswit.dictionaries.Dictionary.CAREER_PATH,
            "Authz $marker",
        )

        // Self-edit assigning a career field is ADMIN-only → 403.
        assertEquals(
            HttpStatusCode.Forbidden,
            client.put("/api/v1/users/$id") {
                contentType(ContentType.Application.Json)
                setBody(UserUpdateRequest(name = "Plain", email = email, roles = emptyList(), careerPathId = pathId))
            }.status,
        )
        // Authz before validation: an invalid email alongside the change still answers 403, not 400.
        assertEquals(
            HttpStatusCode.Forbidden,
            client.put("/api/v1/users/$id") {
                contentType(ContentType.Application.Json)
                setBody(UserUpdateRequest(name = "Plain", email = "not-an-email", roles = emptyList(), careerPathId = pathId))
            }.status,
        )
        // Omitting the fields entirely stays an ordinary self-edit → 204.
        assertEquals(
            HttpStatusCode.NoContent,
            client.put("/api/v1/users/$id") {
                contentType(ContentType.Application.Json)
                setBody(UserUpdateRequest(name = "Plain Renamed", email = email, roles = emptyList()))
            }.status,
        )
    }

    @Test
    fun `non-admin self PUT resubmitting their current career field succeeds`() = testApplication {
        usePostgresTestcontainer()
        val adminEmail = uniqueEmail("career-authz-admin")
        TestUsers.seed(email = adminEmail, password = "pw-123456789")
        val email = uniqueEmail("career-holder")
        val id = TestUsers.seed(email = email, password = "pw-123456789", roles = emptySet())
        val marker = java.util.UUID.randomUUID().toString().take(8)
        val (pathId) = TestDictionaries.append(
            ch.nokillswit.dictionaries.Dictionary.CAREER_PATH,
        ...
        // Admin assigns the field…
        authedClient(adminEmail, "pw-123456789").put("/api/v1/users/$id") {
            contentType(ContentType.Application.Json)
            setBody(UserUpdateRequest(name = "Holder", email = email, roles = emptyList(), careerPathId = pathId))
        }.let { assertEquals(HttpStatusCode.NoContent, it.status) }

        // …and the user may echo it back unchanged (a full-representation PUT), not being admin.
        assertEquals(
            HttpStatusCode.NoContent,
            authedClient(email, "pw-123456789").put("/api/v1/users/$id") {
                contentType(ContentType.Application.Json)
                setBody(UserUpdateRequest(name = "Holder Renamed", email = email, roles = emptyList(), careerPathId = pathId))
            }.status,
        )
    }
