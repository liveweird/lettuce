...
class TeamMemberSubordinateStatsTest {
    ...
    fun `pre-audit-trail feedbacks fall back to lastModified, shared teams repeat, indirect reports enrich`() = testApplication {
        ...
        val callerId = TestUsers.seed(email = callerEmail, password = "pw", name = "Caller", role = UserRole.USER)
        val subId = TestUsers.seed(email = uniqueEmail("sub"), password = "pw", name = "Sub", role = UserRole.USER)
        val otherSubId = TestUsers.seed(email = uniqueEmail("sub2"), password = "pw", name = "Sub2", role = UserRole.USER)
        val indirectId = TestUsers.seed(email = uniqueEmail("ind"), password = "pw", name = "Ind", role = UserRole.USER)
        val admin = authedClient(adminEmail, "pw")
