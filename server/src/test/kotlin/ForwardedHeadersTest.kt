 ...
 * appends instead of replacing (v3.6.2). Every attempt uses a distinct unknown email so the
 * per-account lockout (5 per email) never trips, and the login bucket is pinned to 10 like
 * RateLimitResponseTest; every testApplication boots a fresh app, so buckets never leak.
 */
