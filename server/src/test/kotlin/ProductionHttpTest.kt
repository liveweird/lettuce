    ...
    fun `production mode redirects plain HTTP with the security headers and honors X-Forwarded-Proto`() {
        val server = EngineMain.createServer(
            arrayOf(
                ...
                "-P:security.encryption.key=${strongEncryptionKey()}",
        ...
        try {
            ...
            fun get(vararg headers: String): HttpResponse<String> {
                val request = HttpRequest.newBuilder(URI("http://127.0.0.1:$port/")).GET()
