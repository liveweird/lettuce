 ...
 * SPA renders each one in the viewer's language. Params carry enum names and numeric values only —
 * NEVER title/description/summary text (description and summary are encrypted at rest;
 * team_kpi_events.params is plaintext).
 */
...
/**
 * Structured event recorded on an ACTIVE progress update, with the from/to values — the KPI
 * view's Graph tab derives its value-over-time series from exactly these events. Returns null
