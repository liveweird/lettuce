...
/**
 * Days-off entries (see V40/V77), their budget corrections (V42), and — since v3.2.0/V74 — the
 * paid pools: the ORG-WIDE kinds registry ([PoolTypes], ADMIN-managed) and the PER-USER grants
 * ([Pools], a chain manager's right). No approval lifecycle (v3.9.0/V77 — every user creates an
 * entry directly; it exists until soft-deleted, no accept/reject/cancel). One encrypted-at-rest
 * column remains (the 1:1-notes pattern — the cipher wraps every write and unwraps every read;
 * never filter/sort on it in SQL): the correction COMMENT. Entry dates, cost, and pool are
 * immutable after create — only `marked_as_deleted` ever changes. Every PAID entry and every
 * correction references its pool KIND (never the grant row), so a pool's history is keyed on
...
class DaysOffService(val database: R2dbcDatabase, private val cipher: ch.nokillswit.infra.crypto.FieldCipher) : EncryptedAtRest {
    ...
    /**
     * Validates state rules and inserts a new entry, atomically: the overlap invariant (the
     * owner's active periods must not intersect — [ConflictException] with `instance` pointing
     * at the existing entry, the feedback-duplicate precedent), the working-day cost against the
     * CURRENT holiday registry (frozen on the row; a zero-cost period — weekends/holidays only —
     * is 400), and for PAID the budget sweep: with the new cost added, [remainingHalfDays] must
     * stay >= 0 for every year from the entry's through the last year holding an active PAID
     * entry (a retroactive create must not push a later, carry-over-funded year negative) —
     * else 409. Returns the new id plus the create fan-out notifications to persist (v3.9.0 —
     * every non-deleted person sharing a team with the owner, plus each such team's direct
     * manager, minus the acting person).
     *
     * With [recordedBy] (v2.29.0, the on-behalf flow — the route has already verified the actor
     * is a manager in [userId]'s TRANSITIVE chain) the row is owned by [userId] but the acting
     * manager is excluded from their own fan-out receipt instead of the owner. Every state rule
    ...
    suspend fun create(
        ...
        suspendTransaction(database) {
            val overlapping = Requests
                .select(Requests.id)
                .where {
                    (Requests.userId eq userId) and active() and
            ...
            if (overlapping != null) {
                throw ConflictException(
                    "The period overlaps an existing days-off entry",
            ...
            // history only; a non-carry-over pool resets every January, so only the entry's
            ...
            if (pool != null) {
                ...
                val usedByYear = activePaidCostsByYear(userId, kind.id).toMutableMap()
            ...
            val actorId = recordedBy ?: userId
            val notifications = daysOffFanoutNotifications(
                type = NotificationType.DAYS_OFF_CREATED,
                recipientIds = fanoutRecipientIds(userId, actorId),
                person = userName(userId),
    ...
    suspend fun read(id: UInt): DaysOffResponse? = suspendTransaction(database) {
        joined()
            ...
            .map { toFullResponse(it) }
            .singleOrNull()
    }

    /** A joined() row → the full response. */
    private fun toFullResponse(row: ResultRow): DaysOffResponse =
        DaysOffResponse(
     ...
     * userId/type/startDate-bound predicate.
     */
    suspend fun listAllFull(filter: DaysOffListFilter, paging: PageRequest): DaysOffFullListResult =
        suspendTransaction(database) {
            ...
            val rows = joined().selectAll()
                ...
                .map { toFullResponse(it) }
                .toList()
            DaysOffFullListResult(items = rows, total = total)
        }

    /**
     * Batch for the integration API (v3.0.0): owner id → their entries, optionally bounded to
     * startDates of one calendar year; owners without entries are absent from the map.
     */
    ...
    /**
     * Soft-deletes an entry (v3.9.0 — the sole terminal state now); the route has already
     * authorized [actorId] (the owner or a manager in their transitive chain). Returns the
     * delete fan-out notifications to persist, or null when the row is missing (→ 404).
     */
    suspend fun delete(id: UInt, actorId: UInt): List<Notification>? = suspendTransaction(database) {
        ...
        val updated = Requests.update({ (Requests.id eq id) and (Requests.markedAsDeleted eq false) }) {
            it[markedAsDeleted] = true
            it[lastModified] = System.currentTimeMillis()
        }
        if (updated == 0) return@suspendTransaction null
        daysOffFanoutNotifications(
            type = NotificationType.DAYS_OFF_DELETED,
            recipientIds = fanoutRecipientIds(ownerId, actorId),
            person = userName(ownerId),
            startDate = row[Requests.startDate],
            endDate = row[Requests.endDate],
        )
    }
    ...
    suspend fun list(
        ...
        // canDelete capability flag (chain-wide since v2.33.0); a caller managing nobody pays
        // a single empty-frontier query.
        ...
        val scope: Op<Boolean> = when (view) {
            ...
            // Auditor view (HR-only, gated route-side via requireAuditListAccess): every entry
            // of the target. The route guarantees a non-null userId.
        ...
        val rows = join
            ...
            .map { row ->
                DaysOffListItem(
                    ...
                    days = row[Requests.costHalfDays] / 2.0,
                    createdAt = row[Requests.createdAt],
                    canDelete = row[Requests.userId].value == callerUserId || row[Requests.userId].value in subtree,
                    lastModified = row[Requests.lastModified],
                )
            }
            .toList()
    ...
    suspend fun calendar(
        ...
        val entriesByUser: Map<UInt, List<DaysOffCalendarEntry>> = if (userIds.isEmpty()) {
            ...
            Requests
                ...
                .select(
                    Requests.id, Requests.userId, Requests.type,
     ...
     * (user, pool kind): the closed-form carry-over over that pool's active PAID entries up to
     * [year] (one grouped fetch per table, joined in Kotlin on (user, kind) — no SQL arithmetic
     * on R2DBC). Per user: the default kind's row ALWAYS (allowance null when ungranted —
     * today's "not configured = zero budget"), then one row per active extra grant, then
     * history-only rows (`poolArchived`) for extra kinds with an entry or correction IN [year]
     * but no active grant; extras sort by kind name. Users without any entries still get
        ...
        UserService.Users
            ...
            .map { row ->
                ...
                orderedKinds.map { kind ->
                    ...
                    val activeRows = rowsByUserPool[userId to kind.id].orEmpty()
                    val usedByYear = activeRows.groupBy { it.year }.mapValues { (_, r) -> r.sumOf { it.costH } }
                    val corrections = correctionsByUserPool[userId to kind.id].orEmpty()
                    val usedH = activeRows.filter { it.year == year }.sumOf { it.costH }
                    DaysOffBudget(
                        userId = userId,
                        userName = userName,
     ...
     * legacy plaintext — including soft-deleted rows (the request `cancel_reason` column this
     * used to also sweep was dropped by V77, along with the lifecycle it belonged to). With
     * [reencryptAll] (key rotation) every row is rewritten under the current key. Idempotent;
    ...
    /**
     * Per user, the start date of their next active vacation that hasn't ended yet (v3.9.0 —
     * renamed from nextAcceptedVacationsByUserIds, no status to filter on) — an ongoing one
     * counts (its start still answers "when"). Users with none are absent from the map. Backs
     * the /teams/members card enrichment (managed + member views — teammate-visible by
     * calendar parity).
     */
    suspend fun nextVacationByUserIds(
        userIds: Set<UInt>,
     ...
     * right, and since v2.33.0 (the chain rule) also the delete/record/correction/allowance
     * write rights. */
     ...
     * with [ownerId] (the owner included — [teamMemberPeers]) UNION each of those teams' direct
     * manager ([directManagerIds] — reused rather than re-derived), minus [actorId] and minus
     * any soft-deleted user. Runs in the caller's transaction.
     */
    private suspend fun fanoutRecipientIds(ownerId: UInt, actorId: UInt): Set<UInt> {
        val candidates = (teamMemberPeers(ownerId) + directManagerIds(ownerId)) - actorId
        if (candidates.isEmpty()) return emptySet()
        return UserService.Users
            .select(UserService.Users.id)
            .where { (UserService.Users.id inList candidates) and (UserService.Users.markedAsDeleted eq false) }
    ...
    /** Year → summed half-day cost of [userId]'s active PAID entries in ONE pool kind (every
     * year). */
    private suspend fun activePaidCostsByYear(userId: UInt, poolTypeId: UInt): Map<Int, Int> =
        Requests
