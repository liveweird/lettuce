...
-- DRAFT <-> ACTIVE <-> ARCHIVED (both directions, never skipping ACTIVE — the goals machine, V25);
...
CREATE TABLE team_kpis (
    ...
    current_value      DOUBLE PRECISION NOT NULL,  -- denormalized: the max-dated team_kpi_values row (V30), 0.0 when none
    current_value_date VARCHAR(10)      NULL,      -- ISO date of that row; NULL = no data points; recomputed on every values mutation
    status             VARCHAR(20)      NOT NULL CHECK (status IN ('DRAFT', 'ACTIVE', 'ARCHIVED')),
    summary            TEXT             NULL,      -- encrypted at rest; set by the archive action only
